...
PRIV_TIMER_CHECK:
    			//... [FILL IN CODE HERE ]
                CMP      R5, #29         //check if interrupt caused by timer
                BNE      KEYS_CHECK      //if not, check KEYs
                BL       PRIV_TIMER_ISR    
                B        EXIT_IRQ			
				//
...
KEYS_CHECK:
    			//... [FILL IN CODE HERE ]
                CMP		R5, #73      //check if interrupt caused by KEYs          
UNEXPECTED:
                BNE    UNEXPECTED     //if none of the above
                BL     KEY_ISR				
				//
